...
- **Focus Mode:** `@AppStorage("focusMode")` hides the linter + formatting bar everywhere; on macOS `MacRootView` also sets `columnVisibility = .detailOnly` while focused. With the sidebar collapsed and no tab bar, ⌘1…9 / ⇧⌘[ / ⇧⌘] are the only way to switch documents in Focus Mode.
...
- **Markdown rendering (preview):** **MarkdownUI** (`MarkdownDocumentView`), themed via `MarkdownUI.Theme.vera(fontSize:)`. `parseDocSegments` splits fenced code blocks and pipe-tables out of the stream first, so those render through `CopyableCodeBlock` (Highlightr, atom-one-light/dark) and `DocTableBlock` instead of MarkdownUI. The parse runs off the main actor and is cached per document. *(Corrected 2026-07-26: this entry previously described a custom `MarkdownAttributedString` + `PreviewTextView` and claimed MarkdownUI was unused — neither symbol exists.)*
...
- **Smart Anchor:** Proportional tap-to-offset mapping, inline in `HighlightingTextView` (the scroll-anchor restore blocks in both representables). *(Corrected 2026-07-26: there is no `SmartAnchorResolver.swift`.)*
...
- **File switching:** `.id(source)` on `DocumentOrImageView` still recreates the *view* per document, so its per-document `@State` resets cleanly — but the `EditorViewModel` now lives on the tab and survives, so no re-read and no spinner. `DocumentView` calls `loadIfNeeded()`, plus `reloadIfChangedOnDisk()` to pick up external edits the old accidental re-read used to catch.
...
- **Tabs:** one `EditorViewModel` per tab, **owned by `FileTreeViewModel.TabEntry`** so it outlives the view and a tab switch does no file I/O; text for the least recently activated tabs beyond 8 is evicted and re-read on return, never for a document with unsaved work. **There is no tab bar** — open documents live only in the sidebar's "Open Files" section (deleted 2026-07-26). `NSWindow.allowsAutomaticWindowTabbing = false` + `.handlesExternalEvents(matching: [])` suppress system window-tab chrome and duplicate window creation on macOS; opening a URL already open navigates to that tab.
...
- **iOS file list navigation:** File rows use `Button` with `.buttonStyle(.plain)` calling `openFileInNewTab` directly -- `List(selection:)` only fires `onChange` inside `NavigationSplitView`, not `NavigationStack` on iPhone. *(Corrected 2026-07-26: `openFileInActiveTab`/`openInActiveTab` had zero call sites and were deleted.)*

## SPM dependencies

- **Highlightr** (`https://github.com/raspu/Highlightr`) — syntax highlighting in the editor (`HighlightingTextView`, via `CodeAttributedString`) and in preview code blocks (`HighlightedCodeView` through the shared `HighlightrEngine` actor, capped at 1 MB)
- **MarkdownUI** (`https://github.com/gonzalezreal/swift-markdown-ui`) — **the Markdown preview renderer** (`MarkdownDocumentView`). Note it applies Apple's `.body` Dynamic Type ramp itself via `ScaledMetric`, which is why it is the one caller handed an *unscaled* size by `Theme.Typography`.
