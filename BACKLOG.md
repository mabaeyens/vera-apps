...
- **Manual walk of the Release 1.2.0 checklist** — repo search, conflict recovery, branch switching, branch picker, multi-file commits, and Device Flow sign-in all need a real on-device pass per the new section in TEST_PLAN.md; the 2026-07-06 code-review fixes were verified by build + direct code reading, not a full manual click-through.
- **iPhone Miguel sideload failing** — `/vera-validate` on 2026-07-06 got "the developer disk image could not be mounted" for the physical-device build step. Device/connection issue (reconnect via USB, confirm the trust dialog), not a code problem — simulator and macOS builds both passed clean.
